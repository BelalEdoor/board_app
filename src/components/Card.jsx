    ...
    >
      ...
      <div className={styles.actions} style={{ borderColor: color.text + '33' }}>
        <button
          ...
          onClick={() => toggleLike(card.id, currentUser)}
        ...
        <div className={styles.emojiWrapper}>
          <button
            className={`${styles.actionBtn} ${myReaction ? styles.reacted : ''}`}
          ...
          {showEmojis && (
            <div className={styles.emojiPicker} onClick={e => e.stopPropagation()}>
              {EMOJIS.map(e => (
                <button key={e} className={styles.emojiOption}
                  onClick={() => { addReaction(card.id, currentUser, e); setShowEmojis(false) }}>
                  {e}
                </button>
              ))}
            </div>
